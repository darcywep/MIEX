...
contract MinimalSmallBank {
    ...
    /// @notice 根据账户余额计算 Fibonacci(n)，并更新余额为 (fib/2) * 1e18
    /// @param user 要操作的账户地址
    /// @param n Fibonacci 序列参数 n
    function fibonacciCalculate(address user, uint256 n) external nonReentrant {
        ...
        emit FibonacciCalculated(user, n, fib);
        uint256 newBalance = fib + bal;
        balances[user] = newBalance;

        emit Deposit(user, newBalance); // 作为余额更新的标志
    }
}
