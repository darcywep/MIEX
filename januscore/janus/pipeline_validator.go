...
// 2. 传递性冲突：如果 tx_i 有写，tx_j 有读或写，则 tx_i → tx_j
// 3. 边的类型始终为 WR/WAW（简化处理，用于最大提交验证）
func (pe *PipelineEngine) buildConflictEdges(txOps []*TxOperation, addr string, dag *ConflictDAG) {
	...
	// 对于每对交易 (i, j)，如果 i < j 且 tx_i 有写，tx_j 有读或写，则添加边
	for i := 0; i < n; i++ {
		if !txOps[i].HasWrite {
		...
		for j := i + 1; j < n; j++ {
			// tx_i 有写，tx_j 有读或写，产生 RAW/WAW 冲突。
			if txOps[j].HasRead || txOps[j].HasWrite {
