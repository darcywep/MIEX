...
func printUsage() {
	...
	fmt.Println("  ethereum    真实以太坊负载实验；从 LatencyDB 读取从 21000001 开始的真实 tx latency/rw，默认按原始以太坊区块执行 10000 个区块。")
	fmt.Println()
	...
	fmt.Println("  -baseline  all | harmony | schain | serial | optme | optme_paper | aria | janus | Non_Maximum_Commit_Validation | newHarmony | mvschedo | quecc | pilotfish")
	...
	fmt.Println("  go run ./experiment ethereum -baseline all -t 8 -b 10 -bt 2000 -latency 50")
	fmt.Println()
	fmt.Println("Ethereum flags:")
	fmt.Println("  -baseline  all | harmony | schain | serial | optme | optme_paper | aria | janus | Non_Maximum_Commit_Validation | newHarmony | mvschedo | quecc | pilotfish")
	fmt.Println("             选择要运行的 baseline；默认 janus。ethereum 模式下 harmony 使用 new_harmony，newHarmony 仅作为兼容别名。")
	fmt.Println("  -t         worker thread number；默认 8。")
	fmt.Println("  -b         blocks number；默认 10000。不传 -bt 时表示原始以太坊区块数；传 -bt 时表示重组后的实验区块数。")
