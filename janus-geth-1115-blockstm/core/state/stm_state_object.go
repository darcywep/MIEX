...
func newStateAccount(data *types.StateAccount, statedb *StmStateDB, addrHash common.Hash, address common.Address) *SStateAccount {
	...
	if !bytes.Equal(data.CodeHash, types.EmptyCodeHash.Bytes()) {
		code := statedb.statedb.GetCode(address)
		// 替换成从原始的statedb中读取
...
type stmStateObject struct {
	...
	multiVersionStateMutex sync.RWMutex
	multiVersionState      map[int]*stateOperation // map[int]*stateOperation
...
func (op *stateOperation) DeepCopy() *stateOperation {
	if op == nil {
		return nil
	}
	n := &stateOperation{
	...
	if op.account != nil {
		n.account = op.account.Copy() // 你已有/会实现的
	}
	return n
}
...
func (op *storageOperation) DeepCopy() *storageOperation {
	if op == nil {
		return nil
	}
	return &storageOperation{
		incarnation:  op.incarnation,
		storageValue: op.storageValue, // Hash 是值类型，直接拷贝即可
		estimate:     op.estimate,
	}
}
...
func (m slotMaps) DeepCopy() slotMaps {
	if m == nil {
		return nil
	}
	n := make(slotMaps, len(m))
	for k, v := range m {
		if v != nil {
			n[k] = v.DeepCopy() // deep copy slotMap
		} else {
			n[k] = nil
		}
	}
...
func (m slotMap) DeepCopy() slotMap {
	if m == nil {
		return nil
	}
	n := make(slotMap, len(m))
	for k, v := range m {
		if v != nil {
			n[k] = v.DeepCopy()
		} else {
...
func newStmStateObject(db *StmStateDB, address common.Address, data types.StateAccount) *stmStateObject {
	...
	return &stmStateObject{
		...
		data:                newStateAccount(&data, db, crypto.Keccak256Hash(address[:]), address),
		multiVersionState:   make(map[int]*stateOperation),
...
func (s *stmStateObject) DeepCopy(stmStateDB *StmStateDB) *stmStateObject {
	if s == nil {
		return nil
	}
	...
	ns := &stmStateObject{
		address:  s.address,
		addrHash: s.addrHash,
		db:       stmStateDB, // 通常不 deep copy DB，此处按需修改
	}

	// ----- deep copy SStateAccount -----
	if s.data != nil {
		ns.data = s.data.Copy() // 你需要实现
	}

	// ----- deep copy Trie -----
	if s.trie != nil {
		ns.trie = s.db.db.CopyTrie(s.trie)
	}

	// ----- deep copy multiVersionState -----
	s.multiVersionStateMutex.RLock()
	if s.multiVersionState != nil {
		ns.multiVersionState = make(map[int]*stateOperation, len(s.multiVersionState))
		for k, v := range s.multiVersionState {
			if v != nil {
				ns.multiVersionState[k] = v.DeepCopy() // 请实现 stateOperation.DeepCopy
			} else {
				ns.multiVersionState[k] = nil
			}
		}
	}
	s.multiVersionStateMutex.RUnlock()

	// ----- deep copy multiVersionStorage -----
	s.mvStorageMutex.RLock()
	ns.multiVersionStorage = s.multiVersionStorage.DeepCopy() // slotMaps.DeepCopy() 需要你实现
	s.mvStorageMutex.RUnlock()

	// ----- deep copy dirtyStorage -----
	ns.dirtyStorage = s.dirtyStorage.Copy() // StmStorage.DeepCopy() 需要你实现

	// ----- copy bool -----
	ns.dirtyMarker = s.dirtyMarker
...
func (s *stmStateObject) setState(index, incarnation int, writeSet *WriteSet) {
	...
	s.multiVersionStateMutex.Lock()
	s.multiVersionState[index] = op
	s.multiVersionStateMutex.Unlock()
}
...
func (s *stmStateObject) setStateEstimate(index int) {
	...
	//}
	s.multiVersionStateMutex.Lock()
	defer s.multiVersionStateMutex.Unlock()
	op, ok := s.multiVersionState[index]
	if ok {
		op.estimate = true
	}
}
...
// GetCommittedState 修改为从statedb中读取
func (s *stmStateObject) GetCommittedState(db Database, key common.Hash) common.Hash {
	return s.db.statedb.GetState(s.address, key)
}
...
func (s *stmStateObject) updateAccount() {
	...
	//}
	s.multiVersionStateMutex.RLock()
	for id, _ := range s.multiVersionState {
		txIndexes = append(txIndexes, id)
	}
	sort.Sort(sort.Reverse(sort.IntSlice(txIndexes)))
	for _, index := range txIndexes {
		if index > -1 {
			latestOp, _ := s.multiVersionState[index]
			s.dirtyMarker = true
			s.data = latestOp.account
			break
		}
