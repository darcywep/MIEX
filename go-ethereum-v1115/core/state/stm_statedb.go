...
func (r *ReadLoc) DeepCopy() *ReadLoc {
	if r == nil {
		return nil
	}
	...
	// Deep copy Location
	if r.Location != nil {
		n.Location = r.Location.DeepCopy()
	}

	// Deep copy Version (TxInfoMini)
	if r.Version != nil {
		n.Version = r.Version.Copy() // 你需要提供 TxInfoMini.DeepCopy()
	}
...
func (l *Location) DeepCopy() *Location {
	...
	cp := *l // 所有字段都是值类型或 bool，直接结构体复制即可
	return &cp
}
...
func NewStmStateDB(root common.Hash, db Database, snaps *snapshot.Tree, statedb *StateDB) (*StmStateDB, error) {
...
func (s *StmStateDB) ReadStateVersion(addr common.Address) {
	s.readStateVersion(addr, -1)
}
...
func (s *StmStateDB) readStateVersion(addr common.Address, index int) *ReadResult {
	...
	if obj := s.getDeletedStateObject(addr); obj != nil {
		...
		//	return true
		//})
		obj.multiVersionStateMutex.RLock()
		for id, _ := range obj.multiVersionState {
			txIndexes = append(txIndexes, id)
		}
		obj.multiVersionStateMutex.RUnlock()
		...
		if len(txIndexes) == 0 {
			...
			obj.multiVersionStateMutex.Lock()
			obj.multiVersionState[-1] = op
			obj.multiVersionStateMutex.Unlock()
			res.DirtyState = op
			return res
		}
		...
		//if index == 0 {
		//	fmt.Println("tx index", index, "addr", addr, "maxIndex", maxIndex)
		//}
		//v, ok := obj.multiVersionState.Load(maxIndex)
		//if !ok {
		//	fmt.Println(txIndexes, maxIndex, len(txIndexes), v)
		//}
		//op := v.(*stateOperation)

		obj.multiVersionStateMutex.RLock()
		op, _ := obj.multiVersionState[maxIndex]
		obj.multiVersionStateMutex.RUnlock()
		if op.estimate == true {
...
func (s *StmStateDB) ReadStorageVersion(addr common.Address, hash common.Hash, index int) {
	s.readStorageVersion(addr, hash, index)
}
...
func (s *StmStateDB) ValidateReadSet(index int) bool {
	...
	//w, _ := s.lastWrites.Load(index)
	res := v.([]*ReadLoc)
	//resW := w.([]*Location)
	//if index == 0 {
	//	for _, write := range resW {
	//		fmt.Println("ValidateReadSet write loc:", write.stateAddress, write.storageSlot, write.storageMarker)
	//	}
	//	for _, read := range res {
	//		fmt.Println("ValidateReadSet read loc:", read.Version.Index, read.Version.Incarnation, read.Location.stateAddress, read.Location.storageSlot, read.Location.storageMarker)
	//	}
	//}
	for _, read := range res {
		...
		//if index == 0 {
		//	fmt.Printf("ValidateReadSet tx index=%d\n", index)
		//	fmt.Printf("curRes.Version.Index=%d, curRes.Version.Incarnation=%d\n", curRes.Version.Index, curRes.Version.Incarnation)
		//	fmt.Printf("read.Version.Index=%d, read.Version.Incarnation=%d\n", read.Version.Index, read.Version.Incarnation)
		//}
		...
		for _, loc := range prevLocations {
			...
			if !exist {
				...
				if loc.storageMarker {
					...
					obj.multiVersionStateMutex.Lock()
					delete(obj.multiVersionState, index)
					obj.multiVersionStateMutex.Unlock()
				}
				wroteNew = true
			}
...
//				CodeHash: common.CopyBytes(types.EmptyCodeHash.Bytes()),
...
// getDeletedStateObject 从原始的statedb中获取数据
func (s *StmStateDB) getDeletedStateObject(addr common.Address) *stmStateObject {
	// Prefer live objects if any is available
	s.objectMutex.Lock()
	defer s.objectMutex.Unlock()
	...
	// If no live objects are available, attempt to use snapshots
	var data *types.StateAccount
	//fmt.Println(" s.statedb.getStateObject(addr)", addr)
	oldObj := s.statedb.getStateObject(addr)
	if oldObj == nil {
		tools.PanicError("", fmt.Errorf("can not find StateObject"))
	}
	data = &(oldObj.data)
	// Insert into the live set
	newObj := newStmStateObject(s, addr, *data)
...
func (s *StmStateDB) Copy() *StmStateDB {
	...
	// ---- deep copy maps of addresses ----
	for k, v := range s.stateObjectsPending {
		state.stateObjectsPending[k] = v
	}
	for k, v := range s.stateObjectsDirty {
		state.stateObjectsDirty[k] = v
	}
	for k, v := range s.stateObjectsDestruct {
		state.stateObjectsDestruct[k] = v
	}

	// ---- deep copy stateObjects (sync.Map) ----
	s.stateObjects.Range(func(key, value any) bool {

		addr := key.(common.Address)
		obj := value.(*stmStateObject)
		//fmt.Println("copy stateObjects:", addr)

		if obj != nil {
			state.stateObjects.Store(addr, obj.DeepCopy(state))
		} else {
			state.stateObjects.Store(addr, (*stmStateObject)(nil))
		}
		return true
	})

	// ---- deep copy lastWrites (sync.Map) ----
	s.lastWrites.Range(func(key, value any) bool {
		txid := key.(int)
		list := value.([]*Location)

		newList := make([]*Location, len(list))
		for i, loc := range list {
			if loc != nil {
				newList[i] = loc.DeepCopy()
			}
		}
		state.lastWrites.Store(txid, newList)
		return true
	})

	// ---- deep copy lastReads (sync.Map) ----
	s.lastReads.Range(func(key, value any) bool {
		txid := key.(int)
		list := value.([]*ReadLoc)

		newList := make([]*ReadLoc, len(list))
		for i, loc := range list {
			if loc != nil {
				newList[i] = loc.DeepCopy()
			}
		}
		state.lastReads.Store(txid, newList)
		return true
	})

	// ---- copy dbErr (shallow copy is correct) ----
	state.dbErr = s.dbErr
	return state
}
